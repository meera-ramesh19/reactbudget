...
const Charts = () => {
  const [chartData, setChartData] = useState([]);
   ...
   const incomeArr = chartData.filter((ele)=>{
    return ele.type==='income'
   }).map((ele)=>ele.amount)
   const expenseArr = chartData.filter((ele)=>{
    return ele.type==='expense'
   }).map((ele)=>ele.amount)
   const category=chartData.map((ele)=>ele.category)
  const nameArr=chartData.map((ele)=>ele.itemName)
  
  console.log('income=',incomeArr,'expene=',expenseArr,'nameArr=',nameArr)
  ...
  return (
    <div  >
      <ChartMap incomeArr={incomeArr} expenseArr={expenseArr} 
      nameArr={nameArr} category={category}/>
    </div>
  );
};
